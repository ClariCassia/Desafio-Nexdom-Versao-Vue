    ...
    <nav class="bg-[#05232A] text-white shadow-lg border-b border-[#46C38F]/30">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center md:justify-end gap-3 sm:gap-6 text-sm font-semibold tracking-wide">
          <NuxtLink to="/" class="text-[#46C38F] border-b-2 border-[#46C38F] pb-1 transition-all whitespace-nowrap">
            Validar Guia
          </NuxtLink>
          <NuxtLink to="/historico" class="text-slate-300 hover:text-white transition-all pb-1 whitespace-nowrap">
        ...
        <form @submit.prevent="handleEnviar" class="space-y-6" novalidate>
          ...
          <div>
            ...
            <input 
              ...
              placeholder="Informe o código do procedimento. Ex.: 1234" 
              @blur="marcarComoTocado('procedimento')"
              @input="validarFormulario"
              class="w-full px-4 py-3 border rounded-xl outline-none transition-all text-sm font-medium bg-slate-50/50 focus:bg-white"
              :class="[
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select 
                id="sexo" 
                v-model="form.sexo" 
                @blur="marcarComoTocado('sexo')"
                @change="validarFormulario"
                class="w-full px-4 py-3 border rounded-xl outline-none transition-all text-sm font-medium text-slate-700 bg-slate-50/50 focus:bg-white"
                :class="[
